...
[📖 为什么](#为什么做这个项目) · [👥 适合谁](#适合这些人) · [⚡ 安装](#安装) · [🚀 快速开始](#快速开始) · [🧪 实验编程](#实验编程) · [📊 数据分析](#数据分析) · [🎬 Demo](#demo) · [📂 文件结构](#文件结构)

</div>

<br>

- 🔬 一个 idea 要变成能收数据的实验程序，得先学 Python 或 JavaScript 或 MATLAB。
- 📦 实验室祖传代码换台电脑就崩，没人说得清依赖、没人改得动逻辑。
- 📊 统计方法靠习惯选——"大家都用 ANOVA"，审稿人一句质疑就得从头再来。
- 🔁 分析结果只有自己跑得出来，换个环境换个随机种子结论可能就变了。
- ✂️ 做实验和做分析的经常是两拨人——收完数据才发现设计时根本没想好怎么分析。
...
| # | 技能 | 做什么 | 关键细节 |
|---|------|--------|---------|
| ① | **设计编排** `psy-exp-designer` | 把实验想法变成完整设计规范 | 5 阶段渐进确认。Phase 2 生成试次窗口时间线图，每屏时长、按键、RT 起点一目了然。5 道 Gate 硬门禁。38 范式参考 |
| ② | **代码生成** `psy-exp-coder` | 从设计规范生成可运行代码 | 4 层优先级架构。9 项质量门自动拦截：`time.sleep()`、`KbCheck` 测 RT 直接拒绝。12 步代码模板，参数置顶 |
| ③ | **代码审计** `psy-exp-reviewer` | 检查 → 修复 → 再检查，直到通过才交付 | 5 种审查模式。发现问题后与 coder 进入修复循环，每轮修复后重新审计，直到 0 Critical + 0 Major。烟雾测试协议。就绪标签：`ready_for_collection` |
...
| # | 技能 | 做什么 | 关键细节 |
|---|------|--------|---------|
| ④ | **分析设计** `psy-ana-designer` | 从科学问题出发，设计完整分析方案 | 5 阶段渐进确认。Phase 2 确认数据文件组织（多文件/单文件、命名规则、CSV/Excel/TSV）。Phase 3 用 12 维度对比选择最优方法。Config YAML 为唯一事实源 |
| ⑤ | **分析代码** `psy-ana-coder` | 从分析方案生成可重复脚本 | Phase 0 验证 config → 确认 R/Python → 12 步脚本生成。10 项质量门。R：tidyverse/lme4/ggplot2。Python：pandas/statsmodels/seaborn。代码全部 config 驱动 |
| ⑥ | **分析审计** `psy-ana-reviewer` | 检查 → 修复 → 再检查，直到通过才交付 | 4 种审查模式。发现问题后与 coder 进入修复循环，每轮修复后重新审计，直到 0 Critical + 0 Major。统计正确性 + 可重复性审查。就绪标签：`ready_for_publication` |
