---
name: psych-experiment-programming
description: Use for designing and specifying psychological experiments. Guides through a unified 5-phase workflow to produce a complete config YAML, then routes to psych-experiment-coder for code generation. Does NOT generate code itself — the orchestration and specification layer. Supports PsychoPy, jsPsych, and Psychtoolbox with 38 paradigm references. Trigger for 中文/English requests about 心理学实验程序、实验代码、PsychoPy、trial/block结构、刺激呈现、随机化、反应时、条件表、Go/No-go、Navon、Stroop、priming、IAT、N-back、dot-probe、visual search、task switching、rating、stop-signal.
version: 1.0
status: stable
---

# Psychological Experiment Programming
...
Convert psychological experiment ideas into a complete experiment specification (config YAML) through a single, unified workflow. Every design decision accumulates in the **config YAML** — the single source of truth. When the config is complete, this skill routes to `psych-experiment-coder` for code generation.
...
- **Generating or modifying experiment code directly**: That's `psych-experiment-coder`'s job. Route there instead.
- **Code review or production audit**: That's `psych-experiment-code-reviewer`'s job. Route there instead.
...
| 10 | **No code delivery without reviewer pass** | All platforms | All generated code must pass through `psych-experiment-code-reviewer` before data collection. Programming → Coder → Reviewer is a mandatory chain |
...
| **Gate 4** | Phase 5 | All validation rules pass. Rules 1-6 (blocking) must pass before code generation; rules 7-9 are design-level verification. No missing columns, no broken file paths, no ambiguous accuracy coding | Phase 2/3/4 (based on error type, per Phase 5 routing table) |
| **Gate 5** | Phase 5 (before coder) | **Final Design Review**: Full Design Decision Registry presented. User explicitly confirms ALL decisions (including `[ASSUMED]` items) before code generation. No decision left un-reviewed | Phase 2/3/4 (user can correct any decision) |
...
**Step 3: Code generation** — Only after Gate 5 passes (full registry confirmed), route to `psych-experiment-coder` with the completed config. After code is generated, route to `psych-experiment-code-reviewer` for audit. No experiment code is considered complete until it passes reviewer audit with `ready_for_collection` or `ready_after_minor_fixes`.
...
| "帮我写/改/调试实验代码" | Route to [psych-experiment-coder](../psych-experiment-coder/SKILL.md) |
| "帮我检查这段实验代码能不能正式采集" | Route to [psych-experiment-code-reviewer](../psych-experiment-code-reviewer/SKILL.md); full audit |
| Code modification / debugging | Route to [psych-experiment-coder](../psych-experiment-coder/SKILL.md); apply change; show diff |
...
When a user requests any platform, build the config through Phases 1-4, then route to `psych-experiment-coder` in Phase 5. The coder applies the unified Generation Pipeline with that platform's specific mappings.
