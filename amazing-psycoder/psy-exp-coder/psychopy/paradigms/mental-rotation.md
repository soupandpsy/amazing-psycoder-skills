...
> **Config reference**: [mental-rotation](../../../psy-exp-designer/paradigms/mental-rotation.md)
> **Source**: [Pavlovia demos/mental_rotation](https://gitlab.pavlovia.org/demos/mental_rotation) · PsychoPy 2023.2.3

## Experiment Logic

The experiment begins with two instruction screens. The first ("Intructions_1") explains the task: participants will see two rotated letters and must judge whether they are mirror images of each other when in their normal upright position. They press 'm' if the letters are the same (not mirror images) and 'n' if they are mirror images (different). Pressing 'm' advances to the second instruction screen. The second screen ("Intructions_2") shows an example: a rotated 'F' on the left (`FR.png`, orientation 0) and another rotated 'F' on the right (`F.png`, orientation -90). The text explains that the right letter is a mirror image of the left, so pressing 'n' is correct. Pressing 'n' starts the main experiment.

The main trial loop is driven by a condition CSV file (`MentalRot.csv`) loaded via `data.importConditions()` containing columns: `left_im` (left image filename), `right_im` (right image filename), `leftori` (left image orientation), `rightori` (right image orientation), and `corrAns` (correct answer: 'm' or 'n'). Each trial ("trial" routine) displays two letter images side by side using `visual.ImageStim`, each dynamically set from condition columns via `image_L.setImage(left_im)` and `image_R.setImage(right_im)`, with orientations applied via `setOri()`. Response reminders ("n" for non-mirrored/different, "m" for mirrored/same) appear below the images, and a trial counter ("N/total") displays in the bottom-right corner. The participant responds with 'n' or 'm' keys, and the trial is response-terminated.

A `keyboard.Keyboard` component (`key_resp`) begins listening simultaneously with stimulus onset, with its clock reset precisely at screen flip via `win.callOnFlip(key_resp.clock.reset)` for accurate RT measurement. Accuracy is determined by comparing `key_resp.keys` against `corrAns` from the condition file. After each trial, a feedback routine displays "Correct!" (green) or "Incorrect" (red) for 500 ms, using a `visual.TextStim` whose color and text are set in the Begin Routine code based on `key_resp.corr`. A `correct_counter` accumulator increments each trial.

The experiment ends with a final screen ("the_end") showing the total score ("You scored N correct") and waits for spacebar to exit. Per-trial data is saved to the `TrialHandler` via `trials.addData()`, including key pressed, accuracy, RT, and key press duration. The experiment follows a modern function-based structure (`showExpInfoDlg`, `setupData`, `setupWindow`, `setupInputs`, `run`, `saveData`, `endExperiment`, `quit`), uses `iohub` keyboard backend, and supports experiment pausing via `pauseExperiment()`.

## Key Design Patterns

- `visual.ImageStim` with `setImage()` dynamically loading letter images from condition file columns (`left_im`, `right_im`), with `setOri()` applying per-trial rotation angles (`leftori`, `rightori`)
...
- Trial counter display via `trials.thisN` and `trials.nTotal` formatted as `str(trials.thisN) + '/' + str(trials.nTotal)` on a `visual.TextStim`
- Condition file (CSV) with columns for left/right stimulus images, their orientations, and correct answer ('m'/'n')
- `correct_counter` accumulator initialized in Begin Experiment code, incremented each trial, and displayed on final screen
- Feedback via `visual.TextStim` with `setColor()` (green/red) and `setText()` set in Begin Routine based on `key_resp.corr`
- Component stop timing via `tThisFlipGlobal > component.tStartRefresh + 0.5 - frameTolerance` for 500ms feedback offset
- Modern function-based experiment structure with iohub keyboard backend, pause support, and explicit data/quit functions
