...
> **Config reference**: [change-detection](../../../psy-exp-designer/paradigms/change-detection.md)
> **Source**: [Pavlovia demos/change_detection](https://gitlab.pavlovia.org/demos/change_detection) · PsychoPy 2024.1.5

## Experiment Logic

The change detection experiment opens with an instruction screen ("instructions" routine) that explains the task: participants will see six colored squares, then a single test square at one of the previous locations, and must judge whether that color appeared at that specific location. They press Y for yes (same) or N for no (different). The spacebar advances to begin the first block.

The first block is the change detection task proper, driven by a condition file loaded via `data.importConditions('code/sampled_circle_points_with_colors.csv', selection='1')` into a `TrialHandler` with `nReps=1.0` and random method. Each trial row defines six squares via columns `x1`-`x6`, `y1`-`y6`, and `color1`-`color6`, plus a `condition_label` column ("same" or "different") and an `answer` column ("y" or "n"). The "trial" routine displays all six squares simultaneously for 500ms. On each trial, the six `visual.Rect` components are dynamically configured via `setFillColor(colorN)`, `setPos((xN, yN))`, and `setLineColor(colorN)` using the globals imported from the condition file.

After the 500ms encoding display, the "show_test" routine presents a single test square. In its Begin Routine code, the first five square configurations are collected into `possible_sets` and shuffled. For "same" trials, the test position and color come from the same set entry; for "different" trials, the test position comes from one set entry but the color is taken from a different entry. A question text prompt ("Did this color square appear in this location?") appears above the test square, and the `key_resp` keyboard component listens for 'y' or 'n'. Accuracy is determined by comparing `key_resp.keys` to the `answer` column from the condition file. Feedback displays "Correct!" in green or "Incorrect" in red for 500ms.

After completing the detection block, a "next_instr" routine introduces the second phase: localization. In the localization task, participants again see six squares for 500ms, then a second set of six squares where one has changed color. Number labels (1-6) appear on each square, and participants press the corresponding key to indicate which square changed. The localization trials are loaded from `code/localisation_trials_with_test_columns.xlsx` into a separate `TrialHandler` with `nReps=5.0`. Test colors come from columns `testcolor1`-`testcolor6`, and `changed_color_index` provides the correct answer. Feedback follows each localization trial using the same green/red color scheme. The experiment concludes with a "bye" routine displaying "That is the end - goodbye!" for 1 second.

## Key Design Patterns

- Two-phase experiment structure: change detection block followed by localization block, each with its own TrialHandler and condition file
- Dynamic multi-stimulus configuration via CSV columns (`x1`-`x6`, `y1`-`y6`, `color1`-`color6`) mapped to individual `visual.Rect` components using `setFillColor`/`setPos`/`setLineColor`
- `shuffle(possible_sets)` in Begin Routine code to randomize test probe selection, with color swap logic based on `condition_label` ('same' vs 'different')
- `data.importConditions()` with `selection='1'` parameter to specify which sheet/selection to load from CSV files
- Y/N keyboard response for detection phase; 1-6 numeric key response for localization phase
- Two-tone feedback: `setColor(fbcol, colorSpace='rgb')` with green ('green') for correct and red ('red') for incorrect, applied to `fbtextbox`/`fbtextbox_2`
- `deviceManager` pattern (PsychoPy 2024.1+) for hardware device setup via `setupDevices()`, with keyboard devices registered by name (`start_resp`, `key_resp`, `next_resp`, `localisation_resp`)
- Separate TrialHandlers (`change_trials` and `localisation_trials`) with independent data storage via `change_trials.addData()` and `localisation_trials.addData()`
