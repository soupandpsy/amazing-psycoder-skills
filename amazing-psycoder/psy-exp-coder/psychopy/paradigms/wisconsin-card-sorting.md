...
**Config reference**: [wisconsin-card-sorting](../../../psy-exp-designer/paradigms/wisconsin-card-sorting.md)
**Source:** [Pavlovia wisconsin_card_sorting demo](https://gitlab.pavlovia.org/demos/wisconsin_card_sorting) (PsychoPy 2022.1.2)

## Experiment Logic

The Wisconsin Card Sorting Task (WCST) measures cognitive flexibility and set-shifting ability. Participants must sort cards according to one of three possible rules (color, shape, or number), but the rule is never explicitly stated. Instead, participants must infer the correct rule through trial-and-error feedback after each response. After a series of correct trials, the rule changes without warning, and participants must discover the new sorting rule.

The experiment begins with an instructions screen explaining the task, followed by an example screen showing a sample card display. The participant clicks a continue button to proceed past instructions, then clicks on the example image to begin the experiment. The main task is organized as nested loops: an outer "blocks" loop (2 repetitions) iterates over rules defined in `chooseRule.xlsx`, and an inner "trials" loop iterates over cards from `cards.xlsx`. The `useRows` column from the blocks condition file selects subsets of the cards file for each block. After 7 trials in a block (`if trials.thisTrialN == 6`), the inner loop is forced to end.

Each trial begins with a 1-second fixation cross, after which four reference cards appear at the top of the screen (one red dot, two yellow triangles, three green crosses, and four blue stars) and the trial card appears at the bottom center. The trial card image is set via `trial_card.setImage(card)` from the conditions file. Participants respond by clicking on one of the four reference cards using the mouse. The click detection checks `response.clicked_name` against `corrAns`, with fallback positional checks against each card (`one_red_dot.contains(response)` etc.) to handle cases where `clicked_name` fails on some platforms.

After response, a 1-second feedback routine displays "Correct!" or "Incorrect". Accuracy is scored as `corr = 1` if the clicked name matches the correct answer or if the mouse position falls within the correct reference card. Reaction time is recorded in milliseconds from trial start using `rt = round(t*1000)`. Both Score and RT are saved to the experiment handler. The experiment concludes with a 3-second "Thank you" end screen.

The conditions file `chooseRule.xlsx` for the blocks loop contains the current sorting rule and which rows of the cards file to use. The `cards.xlsx` file contains the card image paths and the correct answer (`corrAns`) for each card under the current rule, using column names like `one_red_dot`, `two_yellow_triangles`, `three_green_crosses`, and `four_blue_stars` to specify which reference card is correct.

## Key Design Patterns

- **Nested trial loops with conditional termination:** An outer `blocks` loop (from `chooseRule.xlsx`) wraps an inner `trials` loop (from `cards.xlsx`). The `useRows` column controls which cards are selected via `data.importConditions('cards.xlsx', selection=useRows)`. The inner loop terminates early after 7 trials by setting `trials.finished = True`.
- **Mouse click on multiple stimulus objects:** The response checks whether the mouse click falls within any of the four reference card stimuli using `obj.contains(response)` for each card, comparing against the `corrAns` condition variable from `cards.xlsx`.
- **Fallback accuracy check:** When `response.clicked_name` may fail (e.g., on mobile), positional checks using individual `contains()` calls on each reference card provide a robust fallback for scoring correctness.
- **Stimulus parameters from conditions files via `exec()`:** Condition file columns are unpacked into local variables using `for paramName in thisTrial: exec('{} = thisTrial[paramName]'.format(paramName))`, making `card`, `corrAns`, and `useRows` directly accessible.
- **Frame-accurate timed stimulus onset/offset:** The fixation cross appears at t=0 and is removed after 1 second using `tThisFlipGlobal > fixation.tStartRefresh + 1.0 - frameTolerance`. The trial card and reference cards appear at different times (0s for reference cards, 1s for trial card), creating a sequence of visual presentation.
- **`event.Mouse` with `prevButtonState` debouncing:** Mouse click detection tracks the previous button state each frame and only registers a new click when the state changes from unpressed to pressed (`buttons != prevButtonState` and `sum(buttons) > 0`). The mouse clock is reset at the start of response monitoring.
- **ioHub keyboard backend:** Uses `psychopy.iohub` with `ioConfig['Keyboard'] = dict(use_keymap='psychopy')` and creates the default keyboard with `keyboard.Keyboard(backend='iohub')` for escape key checking.
- **Trial-level RT recording:** Reaction time is continuously updated each frame as `rt = round(t*1000)`, so the final value at response time captures total trial duration in milliseconds.
