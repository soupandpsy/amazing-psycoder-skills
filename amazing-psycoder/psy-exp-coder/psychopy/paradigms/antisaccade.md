...
> **Config reference**: [antisaccade](../../../psy-exp-designer/paradigms/antisaccade.md)
> **Source**: [Pavlovia demos/antisaccade](https://gitlab.pavlovia.org/demos/antisaccade) · PsychoPy 2025.1.0

## Experiment Logic

The antisaccade experiment begins with an instruction screen that adapts dynamically based on the participant's selected response mode. At startup, `expInfo` includes a "response mode" dropdown with three options: "keyboard", "mouse click", and "hover". Each mode receives tailored instructions explaining the task rules: a colored cue (red or green circle) appears on the left or right side of the screen. If the cue is red, the participant must respond toward the opposite side (antisaccade/pro-saccade mapping). If the cue is green, they respond toward the same side as the cue. The instructions are constructed in the Begin Routine code via a multi-branch conditional that sets `instructions_text` accordingly, then applied via `instructions_txt.setText(instructions_text)`.

Each trial is driven by a condition file (`conditions.xlsx`) loaded via `data.TrialHandler2` with `nReps=1.0` and random method. The condition file specifies `target_x` (position of the colored circle, left or right), `target_color` (red or green), `opposite_location_x` (position of the faint opposite marker), `answer` (expected keyboard key: 'left' or 'right'), and `answer_mouse` (expected mouse target: "target" or "opposite_location"). The trial routine has no fixed duration -- it runs indefinitely until a response is made, with `trial.maxDuration = None`. The `target` is a `visual.ShapeStim` circle whose fill color and position are set dynamically from condition columns. A faint `opposite_location` circle (opacity 0.1) marks the alternative response location.

Three response modes are supported simultaneously within the same trial routine. For keyboard mode, `key_resp` listens for 'left'/'right' arrow keys, and correctness is checked against the `answer` column. For mouse click mode, `event.Mouse` tracks clicks on the target and opposite_location shapes via `obj.contains(mouse)`, with correctness determined by whether the clicked object matches `answer_mouse`. For hover mode, an Each Frame code block checks `target.contains(mouse)` and `opposite_location.contains(mouse)` continuously, setting `hover_correct = 1` when the hover matches the expected answer. Any valid response (keypress, click, or hover) immediately ends the trial via `continueRoutine = False`.

In the "feedback" routine, correctness is evaluated with a three-branch conditional that checks which response mode is active: `mouse.corr[0]` for mouse clicks, `key_resp.corr` for keyboard, or `hover_correct` for hover. The feedback text ("Correct!" or "Incorrect") is displayed for 1 second. The experiment ends with a "thanks" routine displaying "Thanks! Goodbye!" for 1 second. Data for all response modalities is saved: keyboard responses to `trials.addData('key_resp.keys/corr/rt/duration')` and mouse trajectories (x, y, leftButton, midButton, rightButton, time, corr, clicked_name) to `trials.addData()`.

## Key Design Patterns

- Multi-modal response collection: keyboard (`key_resp`), mouse click (`event.Mouse` with `obj.contains()`), and hover detection (`target.contains(mouse)`) all active in the same trial routine
- `data.TrialHandler2` with `isTrials=True` for trial iteration; `data.Routine` objects created inline to track per-routine timing and status
- Adaptive instructions: `instructions_txt.setText()` set from a multi-branch conditional based on `expInfo["response mode"]`
- `visual.ShapeStim` with `vertices='circle'` for both target and opposite-location markers; opposite marker uses `opacity=0.1` for faint visibility
- `trial.maxDuration = None` for indefinite-duration trials that end on participant response
- Three-way correctness evaluation in feedback Begin Routine code, selecting the appropriate correctness source (`mouse.corr[0]`, `key_resp.corr`, or `hover_correct`) based on `expInfo["response mode"]`
- `win.callOnFlip(key_resp.clock.reset)` and `win.callOnFlip(key_resp.clearEvents)` for frame-accurate keyboard timing
- Pilot mode support with `prefs.piloting` overrides for windowed mode, participant ID replacement, and logging level
