...
> **Config reference**: [simon](../../../psy-exp-designer/paradigms/simon.md)
> **Source**: [Pavlovia demos/butterfly_simon](https://gitlab.pavlovia.org/demos/butterfly_simon) · PsychoPy 2025.1.0dev137

## Experiment Logic

The Simon task experiment begins with an instruction screen explaining the rules: two circles appear on screen, and participants must select based on color, not location. When a red circle appears, they select the left of the two circles; when a green circle appears, they select the right circle. The location of the colored circle is irrelevant -- only the color determines the correct response. A "NEXT" button (`visual.ButtonStim`) advances past the instructions.

The experiment has a practice phase followed by the main task. The practice phase loads trials from `practice_conditions.xlsx` using `data.TrialHandler2` with `nReps=1.0` and sequential method. Each practice trial is wrapped in an inner `loop_until_correct` loop (`nReps=10.0`, random) that repeats the same practice trial until the participant responds correctly. In the "practice_trial" routine, two `visual.ShapeStim` circles (`left_circle_2` and `right_circle_2`) are configured via `left_color`/`right_color` from the condition columns. The circles are positioned at `(0.5, 0)` and `(-0.5, 0)` respectively in height units. Mouse click detection via `event.Mouse` checks whether clicks fall on either circle using `obj.contains(mouse_2)`. The routine ends on any valid click.

After the practice trial, the End Routine code checks correctness: `if mouse_2.clicked_name[-1] == correct_response` compares the last clicked object name against the expected response. If correct, `loop_until_correct.finished = True` exits the inner loop; if incorrect, the inner loop repeats. A "feedback" routine displays "Correct!" or "Incorrect" for 500ms. Once all practice trials are mastered, an "instructions_2" screen with a "START" button transitions to the main experiment.

The main experiment ("trial" routine) follows the same structure as practice but without the loop-until-correct wrapper. Trials are loaded from `conditions.xlsx` via `data.TrialHandler2` with `nReps=5.0` and random method. Each trial configures `left_circle` and `right_circle` with their respective colors and positions, then waits for a mouse click on either circle. The End Routine code again compares `mouse.clicked_name[-1]` against `correct_response` and saves correctness via `thisExp.addData('correct', correct)`. A feedback routine displays the result for 500ms. The experiment runs 5 repetitions of all condition rows, then ends with `endExperiment()`.

The Simon effect arises from the condition file design: congruent trials pair a color (e.g., red = select left) with a position (red circle appears on the left), while incongruent trials pair a color with the opposite position (red circle appears on the right). The measured RT difference between congruent and incongruent trials constitutes the Simon effect.

## Key Design Patterns

- Nested loop structure: outer `trials_2` (practice trials) containing inner `loop_until_correct` with `loop_until_correct.finished = True` to exit on correct responses
- `visual.ShapeStim` with `vertices='circle'` for response targets at fixed positions `(0.5, 0)` and `(-0.5, 0)`
- Dynamic color configuration via condition file columns: `left_color`/`right_color` set with `setFillColor()` and `setLineColor()`
- Mouse click response collection with `obj.contains(mouse)` hit-testing against clickable target list via `environmenttools.getFromNames([left_circle, right_circle], namespace=locals())`
- Correctness evaluated in End Routine code: `mouse.clicked_name[-1] == correct_response` comparing clicked object name to condition file column
- `visual.ButtonStim` with `buttonClock` and `isClicked`/`wasClicked` tracking for instruction-screen navigation buttons
- Two separate `data.TrialHandler2` instances: `trials_2` (sequential, `practice_conditions.xlsx`) for practice and `trials` (random 5 reps, `conditions.xlsx`) for main task
- `data.Routine` objects tracking per-routine start/stop/forceEnded status with `routines.maxDuration = None` for open-ended timing
