...
> **Config reference**: [delay-discounting](../../../psy-exp-designer/paradigms/delay-discounting.md)
> **Source**: [Pavlovia demos/delay_discount](https://gitlab.pavlovia.org/demos/delay_discount) · PsychoPy 2024.2.4

## Experiment Logic

The experiment begins with an instruction screen ("instructions" routine) that explains the task: participants will be offered two monetary amounts between 1 and 10 pounds, sometimes available immediately and sometimes after a delay, and they must choose which amount they prefer by clicking the option on screen. A START button (`visual.ButtonStim`) is displayed below the instruction text; clicking it advances to the main experiment loop.

The main trial loop is driven by a condition file (`conditions.xlsx`) loaded via `data.importConditions()`. Each row defines one trial with two key columns: `amount1` and `amount2`, which are strings describing the two choice options (e.g., "£5 now" vs "£7 in 3 days"). A `TrialHandler2` shuffles trials randomly with `nReps=1.0`. The delay discounting structure is entirely defined by the condition file -- the ratio of immediate to delayed offers, the amounts, and the delay durations are all parameterized in the spreadsheet.

Each trial ("trial" routine) presents a question prompt ("Which amount would you prefer to take?") centered near the top of the screen, and two response buttons positioned left and right. The button labels are set dynamically from the condition file columns via `button1.setText(amount1)` and `button2.setText(amount2)`. Both buttons use `visual.ButtonStim` with individual `buttonClock` instances, reset on each trial via `win.callOnFlip(button.buttonClock.reset)`. The routine has no fixed duration -- it runs indefinitely until the participant clicks one of the two buttons, at which point `continueRoutine = False` ends the trial. Button click detection uses the `isClicked` / `wasClicked` pattern tracked per-frame, with click timing stored in `timesOn` and `timesOff` lists.

There is no per-trial feedback or accuracy scoring -- the delay discounting task measures preference, not correctness. Click data (number of clicks, click on/off times) for both buttons is saved to the `TrialHandler` via `trials.addData()`. After all trials complete, a goodbye screen ("bye" routine) displays "That is the end! goodbye!" and the experiment ends. The `ExperimentHandler` automatically saves wide-format CSV and pickle files to a `data/` subdirectory.

Per-trial data captured includes: which button was clicked (`button1.numClicks`, `button2.numClicks`), click onset times (`button1.timesOn`, `button2.timesOn`), and click offset times (`button1.timesOff`, `button2.timesOff`). The `ExperimentHandler` also stores routine-level start/stop timestamps for the instructions, each trial, and the bye routine.

## Key Design Patterns

- `data.Routine()` objects with `.tStartRefresh`, `.tStart`, `.tStop` for structured routine lifecycle tracking (2024.2.4+ style)
- `TrialHandler2` with `data.importConditions('conditions.xlsx')` for CSV-driven trial parameterization with `method='random'` and `nReps=1.0`
- `visual.ButtonStim` with per-button `buttonClock` and `win.callOnFlip(button.buttonClock.reset)` for frame-accurate click timing
- Button labels set dynamically from condition file columns via `button1.setText(amount1)` and `button2.setText(amount2)`
- `isClicked` / `wasClicked` per-frame tracking pattern for detecting new button clicks vs. sustained clicks
- `deviceManager` with `hardware.DeviceManager()` for centralized hardware management (2024.2.4+ pattern, no raw ioHub)
- `routineTimer.addTime(-pauseTimer.getTime())` in pause handler for preserving non-slip timing across pause/resume
- Pilot mode support via `core.setPilotModeFromArgs()` with windowed override and pilot indicator

## Code Example

Complete runnable PsychoPy code for the Delay Discount task:
