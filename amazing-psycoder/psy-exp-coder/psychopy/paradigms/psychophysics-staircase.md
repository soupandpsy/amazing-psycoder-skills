...
> **Config reference**: [psychophysics-staircase](../../../psy-exp-designer/paradigms/psychophysics-staircase.md)
> **Source**: [Pavlovia demos/staircase_demo](https://gitlab.pavlovia.org/demos/staircase_demo) · PsychoPy 2020.2.8

## Experiment Logic

The Staircase Demo implements an orientation discrimination staircase procedure, a threshold-estimation method where the difficulty of each trial adapts based on the participant's previous responses. The task presents two Gabor gratings (one on the left at `(-0.3, 0)` and one on the right at `(0.3, 0)`) -- one grating is always at 0 degrees (vertical, the probe), and the other is tilted by an adaptive orientation amount (`this_ori`). The participant's task is to identify which side contains the 0-degree probe by pressing the left or right arrow key.

The experiment begins with an instruction screen that explains the task and waits for a spacebar press to begin. A trial counter (`trialCount`) tracks progress, and on the very first trial (when `trialCount == 0`), the starting orientation is set from `startVal` (70 degrees) and the initial step index (`thisStep`) is set to 0. Before each trial, the orientation is optionally negated if `expInfo['startOri']` is set to `'left'`, causing the oriented grating to tilt leftward. The two orientations (the adaptive orientation and 0 degrees) are placed into a list and shuffled for random left/right assignment, then assigned to `leftGrating` and `rightGrating`. The correct answer (`corrAns`) is determined by which side received the 0-degree probe.

The staircase logic uses parameters modeled after the PsychoPy `StairHandler` class: `nReversals` (5), `stepSizes` ([10, 5, 2, 1, 0.5]), `nTrials` (100 max), `nUp = 1` (one incorrect response increases difficulty), `nDown = 1` (one correct response decreases difficulty), `maxVal` (90 degrees), `minVal` (0 degrees), and `currentDirection` (starts as `'down'`). After each response, the current orientation is adjusted: if the response is correct and `currentDirection` is `'down'`, the orientation decreases by the current step size (making it harder); if the response is incorrect, the direction reverses to `'up'`, the orientation value is appended to `reversalVals`, and if the minimum step size hasn't been reached, `thisStep` increments to the next (finer) step size. The staircase terminates either when the number of reversals reaches `nReversals` (setting `trials.finished = True`) or when `nTrials` is exhausted (via `TrialHandler(nReps=nTrials)`).

Each trial shows the two gratings for 500 ms, with the stimuli starting at t=500 ms (via `tThisFlip >= 0.5-frameTolerance`) and stopping automatically 500 ms later. The keyboard component starts at t=0 ms and listens for `'left'` or `'right'` arrow keys. A fixation cross and a debugging text line (showing the current step size) are displayed throughout. After each trial, response data (keys, RT, correctness, this_ori, currentDirection, corrAns, currentStepSize) are saved to the TrialHandler.

At the end of the experiment, the final threshold is calculated as the average of the last `avRevs` (3) reversal values: `threshold = np.average(reversalVals[-avRevs:])`. An end screen displays the threshold estimate (e.g., "Your threshold was 12.345 degrees!"), shows a grating at the threshold orientation, and provides a clickable "click here to end" button that terminates the experiment via mouse click detection.

## Key Design Patterns

- Simplified staircase algorithm without the `data.StairHandler` class: manually tracks `currentDirection`, `reversalVals`, and `thisStep` index into `stepSizes` for step-size progression
- Adaptive orientation adjustment: correct responses decrease `this_ori` by `stepSize` (making the task harder); incorrect responses reverse direction and increment the step-size index for finer granularity
- Random probe position assignment: `these_oris = [this_ori, 0]` shuffled so the 0-degree probe appears randomly on left or right; `corrAns` determined by which side got the probe
- `TrialHandler(nReps=nTrials, trialList=[None])` creates exactly `nTrials` iterations from an empty trial list -- a pattern for running a fixed number of trials without condition-file-driven parameters
- Early termination via `trials.finished = True` when `len(reversalVals) == nReversals` -- staircase stops at criterion even if `nTrials` not exhausted
- Mouse-based "click to end" using `event.Mouse` with `getPressed()` state-change detection (comparing `buttons != prevButtonState`) and `obj.contains(mouse)` for hit testing
- End-of-experiment threshold averaging: `np.average(reversalVals[-avRevs:])` computes the mean of the last 3 reversal values as the final threshold estimate

## Code Example

Complete runnable PsychoPy code for Staircase Demo task:
