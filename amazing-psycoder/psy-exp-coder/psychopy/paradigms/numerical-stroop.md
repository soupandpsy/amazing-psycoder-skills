...
> **Config reference**: [numerical-stroop](../../../psy-exp-designer/paradigms/numerical-stroop.md)
> **Source**: [Pavlovia demos/numerical_stroop](https://gitlab.pavlovia.org/demos/numerical_stroop) · PsychoPy 2023.1.3

## Experiment Logic

The Numerical Stroop task measures the ability to inhibit an automatic numerical magnitude comparison in favor of a physical size judgment. Participants are shown two numbers side by side and must indicate which side has the physically larger numeral, ignoring the numerical magnitude of the digits themselves. The paradigm is structured around blocks (loaded from `blockDefinitions.xlsx`), each containing a combination of an instruction text, a practice condition file, and a main condition file.

Each block begins with a "BlockInstruct" routine that displays the block-specific instruction text (`instructionText` from the block definition) and waits for a spacebar press to proceed. The instruction tells participants which dimension to respond to (e.g., "which side has the larger number" versus "which side has the physically larger number"), creating the congruency manipulation.

Each trial within a block follows a fixed sequence. First, a fixation cross ("+") appears for 100 ms. Then, at 200 ms into the routine, two numbers (`NumberLeft` and `NumberRight`) appear simultaneously on either side of the fixation location. The numbers are positioned at `(-0.075, 0)` and `(0.075, 0)` in height units. Critically, the trial condition file provides four parameters for each trial: `number1` and `number2` (the digit strings to display), and `size1` and `size2` (the physical font heights in height units, set via `setHeight()`). This allows congruent trials (e.g., larger physical size matches larger numerical value) and incongruent trials (e.g., larger physical size but smaller numerical value).

Response collection begins at the same 200 ms onset as the number stimuli. The keyboard listens for "a" (left side) or "k" (right side) via `Response.getKeys(keyList=['a','k'])`. The correct answer (`corrAns`) is provided by the condition file. A response terminates the trial immediately (response-terminated); if no response is made, the trial remains on screen indefinitely. After each practice trial ("ftrials"), correctness is evaluated by comparing `Response.keys` to `corrAns`, and feedback ("Correct!" or "Oops! That was wrong") is displayed for 1 second. After practice trials, an "EndOfPractice" screen appears, allowing the participant to press spacebar to begin the main trials. Main trials ("trials") follow an identical structure but skip the feedback and proceed directly to the next trial.

The experiment uses a two-level nested loop: an outer `blocks` loop iterates through block definitions, and inner `ftrials`/`trials` loops iterate through practice and main trials respectively. All trial conditions are imported from Excel files specified in the block definition (`practiceFile` and `conditionsFile`). Data is stored using `ftrials.addData()` and `trials.addData()` for response keys, accuracy, RT, and duration.

## Key Design Patterns

- Two-level nested loops: outer `blocks` (TrialHandler) drives block-level instructions and condition files, inner `ftrials`/`trials` loops drive individual trials with separate practice and main trial handlers
- Dynamic font size manipulation via `NumberLeft.setHeight(size1)` and `NumberRight.setHeight(size2)` to create physical size vs. numerical magnitude conflict
- Delayed stimulus onset pattern: fixation at t=0 for 100 ms, then numbers and response component start simultaneously at t=200 ms (`tThisFlip >= 0.2-frameTolerance`)
- Condition files provide `corrAns` (correct key), `number1`/`number2` (digit strings), and `size1`/`size2` (font heights) per trial
- Explicit correctness check: `if (Response.keys == str(corrAns)) or (Response.keys == corrAns)` with a fallback `str(corrAns).lower() == 'none'` check for no-response trials
- Block-level condition file (`blockDefinitions.xlsx`) contains `instructionText`, `practiceFile`, and `conditionsFile` columns to drive different task versions within a single experiment

## Code Example

Complete runnable PsychoPy code for Numerical Stroop task:
