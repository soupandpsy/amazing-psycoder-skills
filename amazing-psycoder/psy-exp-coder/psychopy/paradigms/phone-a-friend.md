...
> **Config reference**: [phone-a-friend](../../../psy-exp-designer/paradigms/phone-a-friend.md)
> **Source**: [Pavlovia demos/phone_a_friend](https://gitlab.pavlovia.org/demos/phone_a_friend) · PsychoPy 2025.2.0

## Experiment Logic

The phone-a-friend experiment presents a trivia-style cue-utilization task modeled after the "Who Wants to Be a Millionaire" lifeline. The experiment opens with an instruction screen over a background image (`rudy-issa-jedKD4yaTvk-unsplash.jpg`) explaining that on each trial participants will be asked a question and can "phone a friend" for a hint up to 10 times total. Participants press space to begin.

Each trial ("trial" routine) displays a question from the condition file (`conditions.xlsx` loaded via `data.TrialHandler2`) in the `this_question_text` textbox. Below the question, an editable textbox (`answer`) allows free-text entry of the answer. A "Phone a friend" button (`hint_button`) is displayed, and a `call_tracker` textbox shows remaining calls (e.g., "Calls remaining: 10"). The trial runs indefinitely (`maxDuration = None`) until either the participant presses Enter (to submit an answer) or clicks the hint button. The `key_resp` keyboard component listens for the Return key; if pressed with text in the answer box, the trial ends and the answer is saved. If the hint button is clicked (detected via `event.Mouse` click tracking on `hint_button`), the trial also ends, transitioning to the "show_hint" routine.

The "show_hint" routine is conditionally skipped: its Begin Routine code checks `if key_resp.keys` -- if a key was pressed (i.e., the participant submitted an answer without requesting a hint), `continueRoutine = False` skips the hint display. Otherwise, the hint counter increments (`n_calls += 1`), remaining calls decrease, and if `n_calls >= limit` (10), the routine is skipped and a `warn` flag is set to trigger the "warn_user" routine. The hint content is determined by randomly sampling from a balanced `cue_types` list (5 valid + 5 invalid cues, shuffled at experiment start). Each trial in the condition file has both `this_cue_valid` and `this_cue_invalid` columns. On each hint request, the last item from `cue_types` is popped (sampling without replacement); if "valid", the valid cue is displayed; if "invalid", the invalid cue is shown. The hint appears as "Answer: [hint text]" in the `friends_hint` textbox alongside the question and an editable answer box (`answer_2`). Participants press Enter to submit their answer.

The "warn_user" routine is shown when all 10 calls are exhausted -- it displays "YOU HAVE NO CALLS LEFT" for 1 second, and the hint button position is moved offscreen (`hint_x = -500`) to prevent further clicking. After the "warn_user" finishes (or if skipped), the next trial begins. Once all condition file trials complete, the experiment shows a "bye" routine with "Thanks for taking part!" and waits for spacebar to exit. Data saved includes the submitted answer text (`answer.text` and `answer_2.text`), key press info, mouse click data (x, y, button states, clicked_name), and critical trial-level variables including `cue_type`, `this_hint`, `n_calls`, and `calls_remaining`.

## Key Design Patterns

- Conditional routine skipping: `show_hint` skips if participant already submitted an answer (`if key_resp.keys: continueRoutine = False`), and `warn_user` only runs when `warn != 0`
- Balanced cue sampling without replacement: `cue_types` list with 5 'valid' and 5 'invalid' entries shuffled at start, then `.pop()` on each request for exactly balanced presentation
- Editable textbox response collection: `visual.TextBox2` with `editable=True` for free-text answer entry, saved via `answer.text`
- Dual response modalities: keyboard (Return key via `key_resp`) and mouse click (on `hint_button` via `event.Mouse` with `obj.contains()`)
- Offscreen element hiding: hint button position set to `hint_x = -500` when calls exhausted, moving it out of the visible viewport
- Counter-based resource limiting: `n_calls` tracks total hint requests, `calls_remaining = limit - n_calls` shown to participant, with hard cap of 10 enforced by the `warn_user` routine
- Background image via `visual.ImageStim` with `size=(1.7, 1)` for full-screen backdrop across all routines
- `data.Routine` objects for structured routine lifecycle tracking with `maxDuration = None` for open-ended trials
