...
> **Config reference**: [choice-reaction-time](../../../psy-exp-designer/paradigms/choice-reaction-time.md)
> **Source**: [Pavlovia demos/choice_reaction_time](https://gitlab.pavlovia.org/demos/choice_reaction_time) · PsychoPy 2021.2.3

## Experiment Logic

The experiment begins with an instructions screen ("instructions" routine) that explains the task: participants must decide which of three shapes they have seen and respond either via keyboard (C for cross, V for square, B for plus) or by clicking the corresponding on-screen response button. Three reference images (`response_square.jpg`, `response_plus.jpg`, `response_cross.jpg`) are displayed at the bottom of the screen as visual button mappings. The participant presses any key or clicks to begin the practice phase.

The practice loop is driven by a condition file (`RTtimeConditions.xlsx`) loaded via `data.importConditions()`. Each row specifies two key columns: `thisImage` (the target image file path, e.g. `images/target_square.jpg`) and `onsetTime` (the variable delay before the target appears). The `TrialHandler` shuffles trials randomly with `nReps=1.0`. Within each trial ("trial" routine), four grey square position tiles appear at the four possible target locations after 0.5 seconds, and the target image appears at a randomly selected position (chosen from a shuffled list of X coordinates: -0.375, -0.125, 0.125, 0.375). The target is displayed briefly for 0.2 seconds and then disappears, while the position tiles and response buttons remain visible until a response is made.

The trial logic dynamically maps each stimulus image to its correct response via an if/elif chain at the start of each trial: `thisImage` determines `corrAns` (keyboard key: 'v', 'c', or 'b'), `corrButton` (the visual button object), and `corrButtonName` (string for mouse accuracy comparison). Both keyboard (`keyResp`) and mouse (`mouseResp`) inputs are active simultaneously. The keyboard checks for keys 'b', 'c', 'v', and on keypress computes accuracy by comparing `keyResp.keys` to `corrAns`. Mouse clicks are validated by checking whether the click location falls within any of the three response button bounding boxes using `obj.contains(mouseResp)`. Either input type ends the trial immediately. Reaction time is calculated relative to the target onset by subtracting `onsetTime` from the response timestamp.

After each practice trial, a feedback routine ("trial_feedback") displays detailed information about the preceding trial: the trial duration, recorded reaction time, response type (keyboard or mouse), and whether the response was correct. The participant presses space or clicks to advance. After completing the practice loop, an instruction screen ("mainTrial_instruct") signals the start of the main experiment. The main experiment loop repeats the same trial structure with `nReps=2.0` (two full repetitions of the condition set). The experiment concludes with an end message displayed for approximately 3 seconds ("end_message" routine) thanking the participant.

Per-trial data including target onset/offset times, keyboard responses (keys, RT, accuracy), mouse responses (x, y, button states, time, clicked object name), calculated trial reaction time, response type, and accuracy are saved to the `TrialHandler` via `PracticeLoop.addData()` and `ExptLoop.addData()`. The `ExperimentHandler` automatically saves CSV and pickle files to the `data/` subdirectory.

## Key Design Patterns

- `data.importConditions('RTtimeConditions.xlsx')` loads trial definitions with `thisImage` and `onsetTime` columns into a `data.TrialHandler` with `method='random'`
- Random target positioning via `shuffle(positionList)` and selecting `positionList[0]` from a list of four pre-defined X coordinates
- Dual input modality (keyboard + mouse) with `keyResp` checking keys 'b'/'c'/'v' and `mouseResp` using `obj.contains()` for hit-testing against clickable button objects
- Dynamic response mapping via if/elif: `thisImage` path determines `corrAns`, `corrButton` object, and `corrButtonName` string
- Target image onset driven by the condition file's `onsetTime` column (variable SOA), displayed for a fixed 0.2s duration then auto-removed
- Reaction time calculated as `keyResp.rt - onsetTime` or `mouseResp.time - onsetTime` relative to target onset, with response type and accuracy recorded per trial
- Separate practice (`nReps=1.0`) and experiment (`nReps=2.0`) loops with a "mainTrial_instruct" transition screen between them
- Position tiles (`rightright_tile`, `right_tile`, `left_tile`, `leftleft_tile`) appear at 0.5s delay before target, providing spatial reference for the four possible target locations

## Code Example

Complete runnable PsychoPy code for Choice Reaction Time task:

```python
# Source: choice_reaction_time (demos/choice_reaction_time)
# Project URL: https://gitlab.pavlovia.org/demos/choice_reaction_time
