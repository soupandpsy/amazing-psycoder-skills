...
> **Config reference**: [sternberg](../../../psy-exp-designer/paradigms/sternberg.md)
> **Source**: [Pavlovia demos/sternberg](https://gitlab.pavlovia.org/demos/sternberg) · PsychoPy 2023.2.3

## Experiment Logic

The experiment begins with a practice instruction screen ("practInstruct") explaining the Sternberg memory scanning paradigm: participants will see a sequence of 1-6 randomly ordered numbers, and after a short delay they will be shown a single probe number. They must decide whether the probe was in the original sequence, pressing LEFT if it was NOT and RIGHT if it WAS. The instructions explain that there will be practice trials with feedback. Pressing any key proceeds.

The practice block ("pracTrials") loads conditions from `pracTrials.xlsx` via `data.importConditions()`, with columns `numberSet` (the memory set as a string), `target` (the probe digit as a string), and `corrAns` (correct answer: 'left' or 'right'). Each practice trial has a precisely timed sequence: a fixation cross (`+`) appears for 1.0 seconds (using `tThisFlipGlobal > fixation.tStartRefresh + 1.0 - frameTolerance`), then the memory set is displayed for 1.5 seconds (starting at `tThisFlip >= 1.2`), followed by a blank delay. After 2 seconds of blank interval, the target probe appears (starting at `tThisFlip >= 4.7`) for up to 2 seconds alongside the keyboard component. The keyboard (`resp`) begins listening at the same time as the probe appears, with `win.callOnFlip(resp.clock.reset)` for frame-accurate RT measurement. A textbox with response reminders appears at `tThisFlip >= 5.7` in case the participant hasn't responded yet.

Accuracy is determined by comparing `resp.keys` against `corrAns` from the condition file. After each trial, a feedback routine displays "Correct! RT=Nms" (with RT rounded to milliseconds) or "Oops! That was wrong" for 1.0 second. After the practice block completes, practice data is saved as an Excel file via `pracTrials.saveAsExcel()`. Then a main instruction screen ("mainInstruct") reminds participants of the rules, and pressing any key advances to the main experiment.

The main experiment block ("trials") loads conditions from `mainTrials.xlsx` with the same structure as practice. The trial routine is identical in timing and logic to the practice trials but without the feedback routine. Data is saved to `trials.addData()` for each main trial, and after completion, trial data is saved as an Excel file. The experiment ends with a "Thanks for taking part!" screen displayed until escape. The experiment follows a modern function-based structure with `showExpInfoDlg`, `setupData`, `setupWindow`, `setupInputs`, `run`, `saveData`, `endExperiment`, and `quit` functions, uses an `iohub` keyboard backend, and supports experiment pausing.

## Key Design Patterns

- Fixed-duration multi-component trial timeline: fixation (1s) -> memory set (1.5s starting at t=1.2) -> delay (2s blank) -> target probe (starting at t=4.7) -> response
- Time-scheduled component start/stop using `tThisFlip >= onsetTime - frameTolerance` and `tThisFlipGlobal > component.tStartRefresh + duration - frameTolerance`
- `keyboard.Keyboard()` with `win.callOnFlip(resp.clock.reset)` synchronized to probe onset for precise RT measurement
- `visual.TextStim` for displaying memory set string (e.g. "3 5 7 1") and target probe from condition file columns
- Practice + main experiment design with separate `TrialHandler` loops (`pracTrials` and `trials`), each with independent condition files
- Trial counter via `visual.TextBox2` set to `str(currentLoop.thisN) + '/' + str(currentLoop.nTotal)`
- Separate feedback routine during practice trials showing RT in milliseconds (`round(resp.rt*1000)`)
- `TrialHandler.saveAsExcel()` to export per-loop data after completion, with `stimOut=params` for condition columns
