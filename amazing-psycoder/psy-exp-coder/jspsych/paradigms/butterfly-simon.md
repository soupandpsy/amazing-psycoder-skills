...
The Butterfly Simon Task is a child-friendly variant of the classic Simon task, using butterfly images instead of abstract shapes. In the Simon task, participants must respond to a stimulus feature (e.g., color) while ignoring its spatial location, which may be congruent or incongruent with the required response hand.

The experiment begins with an instructions screen explaining that participants should press the left key for one butterfly color and the right key for another, regardless of where the butterfly appears on screen. Practice trials with feedback are presented first, followed by the main experimental trials without feedback.

Each trial shows a colored butterfly (e.g., purple, white, or yellow) on either the left or right side of the screen. The participant presses 'z' for one color and 'm' for the other. The critical manipulation is that the butterfly's spatial position may be congruent (e.g., left-positioned butterfly requiring left-hand response) or incongruent (left-positioned butterfly requiring right-hand response).

The experiment uses two condition files: `practice_conditions.xlsx` for practice trials with feedback, and `conditions.xlsx` for the main experimental trials. Reaction time and accuracy are recorded per trial. The typical Simon effect is slower RT for incongruent vs. congruent trials, reflecting response conflict.

## Key Design Patterns

- **Child-friendly stimulus design**: colorful butterfly images (`purple_butterfly.png`, `white_butterfly.png`, `yellow_butterfly.png`) replace traditional abstract shapes
- **Two-phase design**: practice block with feedback followed by main experimental block without feedback, with separate condition files and instruction screens for each
- **Background image** (`background-1877877_1280.jpg`) providing an engaging nature-themed visual context
- **`visual.ImageStim` for stimuli** with dynamic image path set from condition file columns during trial preparation
- **Simon task spatial compatibility logic**: response hand mapping fixed to color while stimulus position varies, creating congruent/incongruent conditions
- **Feedback routine after practice trials** displaying correct/incorrect using `fbtxt.setText()` based on `key_resp.corr`
