...
> **Config reference**: [children-flanker-task](../../../psy-exp-designer/paradigms/children-flanker-task.md)
...
The Children Flanker Task is a child-friendly adaptation of the Eriksen flanker paradigm. Participants see a row of fish images and must respond to the direction of the central fish while ignoring the flanking fish, which may point in the same (congruent) or opposite (incongruent) direction.

The experiment begins with an instructions screen explaining that the child should press the left or right arrow key corresponding to the direction of the middle fish only. Practice trials with feedback are followed by the main experimental trials. A gap screen separates the practice and main phases.

Each trial presents five fish in a horizontal row. The central fish is the target; the four flanking fish point either in the same direction (congruent, e.g., all pointing left) or the opposite direction (incongruent, e.g., target left, flankers right). Fish images (`leftFish.png`, `rightFish.png`) are used, with transparent placeholders for spacing.

The condition file (`conditions.csv`) defines trial parameters including target direction, flanker direction, and the correct response key. A trial counter and accuracy tracker provide progress monitoring. Feedback is presented after each practice trial. The experiment concludes with an end screen. The typical flanker effect is slower and less accurate responses on incongruent vs. congruent trials.

## Key Design Patterns

- **Child-friendly visual design**: fish images (`leftFish.png`, `rightFish.png`) and a colored background appeal to younger participants
- **Two-phase design**: practice trials with feedback followed by main experimental trials, separated by a gap routine
- **Transparent spacer images** (`transparent.png`) to maintain consistent horizontal spacing between fish stimuli
- **Trial counter display** using `visual.TextStim` showing current trial number out of total, providing participant motivation
- **Feedback routine** after practice trials using `visual.TextStim` with text set based on `key_resp_3.corr`
- **Arrow key response mapping**: left and right arrow keys for direction judgments, with condition file `corrAns` column driving accuracy checks
