...
> **Config reference**: [rating-to-choice](../../../psy-exp-designer/paradigms/rating-to-choice.md)
...
The Rating to Choice Task demonstrates how responses from an initial phase can dynamically influence stimuli presented in a subsequent phase. In the first phase, participants rate a set of paintings on a 1-3 scale. In the second phase, they make pairwise choices between paintings based on the ratings they gave.

The experiment begins with instructions. The rating phase presents each painting image individually, and participants press keys 1, 2, or 3 to rate each painting. Ratings are stored with each image filename. After rating all paintings, a second instruction screen explains the choice phase.

In the choice phase, the comparison file (`conditions_choice_phase.xlsx`) specifies which rating comparisons to make (e.g., "1 vs 2", "2 vs 3", "1 vs 3"). The code dynamically selects one painting with each specified rating from the participant's own rating data. If no painting was given a particular rating, a default placeholder image is used instead.

Participants press '1' to choose the left image or '2' to choose the right image. Data collected includes the images displayed, the participant's choice, and the comparison type. This paradigm demonstrates adaptive stimulus selection -- using participant-generated data from one phase to customize stimuli in a later phase.

## Key Design Patterns

- **Adaptive stimulus selection**: phase 2 stimuli are dynamically chosen based on participant's own ratings from phase 1
- **Two-phase design**: rating loop (`trials`) followed by choice loop (`choice_trials`), with instruction screens between phases
- **Custom code for stimulus pairing**: JavaScript code in the choice trial `Begin` routine reads `thisRating` from the comparison file, then searches rating data from phase 1 to find matching paintings
- **Fallback placeholder**: `default.png` used when no painting matches the required rating for a comparison
- **3-key rating scale**: keys 1, 2, 3 mapped to numeric ratings stored in `key_resp.keys`
- **Image-rich stimuli**: paintings from Unsplash and museum collections (European art, nature scenes), loaded as resources
