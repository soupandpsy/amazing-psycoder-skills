...
> **Config reference**: [bart](../../../psy-exp-designer/paradigms/bart.md)
...
The Balloon Analogue Risk Task (BART), based on Lejuez et al. (2002), measures individual differences in risk-taking behavior. Participants are presented with a balloon on screen that they can pump up by pressing a key. Each pump increases the balloon size and adds to their potential reward, but also increases the probability that the balloon will burst.

The experiment begins with an instructions screen explaining the task. A background image provides visual context. The main trial loop is structured as a nested loop: an outer loop iterates over balloons, and within each balloon, an inner loop handles repeated pumps until the participant decides to collect their reward or the balloon bursts.

Each pump trial shows the balloon image expanding (using `visual.ImageStim` with size scaling). The condition file (`conditions.xlsx`) defines parameters including each balloon's burst point. A sound effect (`bang.wav`) plays when a balloon bursts. The participant can press one key to pump and another to collect their current earnings.

The experiment tracks cumulative score across balloons and displays the final score on a concluding screen. Key data collected includes number of pumps per balloon, whether the balloon burst, and total earnings. The BART is typically analyzed by computing the average number of pumps on non-burst trials as a measure of risk-taking propensity.

## Key Design Patterns

- **Nested loop structure**: outer `trials` loop iterates over balloons, inner while-loop (via `continueRoutine`) handles repeated pumps within each balloon
- **Dynamic stimulus scaling**: balloon image size grows with each pump via `balloon.setSize()` in response to key presses
- **Sound feedback**: `sound.Sound` plays a burst sound when the balloon exceeds its threshold, providing salient negative feedback
- **Two-key response system**: one key for "pump" and another for "collect," requiring participants to actively decide when to stop
- **Background image and multiple balloon color assets** (`redBalloon.png`, `blueBalloon.png`, `greenBalloon.png`) for a visually engaging interface
- **Accumulator pattern**: `totalScore` variable initialized in Begin Experiment, incremented per successful pump, and displayed on the final score screen

## Code Example

```javascript
// Source: bart (demos/bart)
// Project URL: https://gitlab.pavlovia.org/demos/bart
// Original file: bart.js
﻿/************* 
 * Bart *
 *************/
