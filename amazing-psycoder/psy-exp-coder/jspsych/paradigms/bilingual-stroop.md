...
> **Config reference**: [bilingual-stroop](../../../psy-exp-designer/paradigms/bilingual-stroop.md)
...
The Bilingual (Blocked) Stroop task compares Stroop interference effects across two languages, demonstrating how automatic word reading can be stronger in one's native language. Participants report the display color of words while ignoring their semantic meaning, which may be congruent or incongruent with the color.

The experiment begins with an intro screen and participant group assignment (Group A or Group B, counterbalancing language order). The task uses a blocked design with two blocks: an English block and a Maori block. Group A sees English words first, while Group B sees Maori words first.

Each trial presents a color word in either English or Maori, displayed in a font color that may match or conflict with the word's meaning. The participant presses one of three keys to indicate the ink color (e.g., 'r' for red, 'g' for green, 'b' for blue). Two separate condition files (`english.xlsx` and `maori.xlsx`) define the stimuli for each language block.

Reaction time is measured from stimulus onset using a keyboard clock reset on flip. Accuracy is determined by comparing `key_resp.keys` to `corr_ans`. A block-level loop (`blocks`) iterates over the two language conditions, with instructions shown at the start of each block. The experiment concludes with a thank-you screen. The key analysis compares the Stroop effect (incongruent RT minus congruent RT) between languages, with a larger effect expected in the more fluent language.

## Key Design Patterns

- **Blocked design with counterbalancing**: two language blocks whose order is determined by participant group assignment (Group A vs Group B)
- **Dual condition files**: separate Excel files for English (`english.xlsx`) and Maori (`maori.xlsx`) stimuli, loaded dynamically based on the current block
- **Outer `blocksLoop`** iterating over language conditions, with inner trial loop presenting stimuli from the appropriate language file
- **Block-level instruction routine** (`blockInstr`) that presents language-specific instructions before each block
- **Three-key color response mapping**: keys 'r', 'g', 'b' mapped to red, green, blue via `key_resp.getKeys({keyList: ['r', 'g', 'b']})`
- **`TrialHandler` method RANDOM** for shuffled presentation within each language block

## Code Example

```javascript
// Source: bilingual_stroop (demos/bilingual_stroop)
// Project URL: https://gitlab.pavlovia.org/demos/bilingual_stroop
// Original file: bilingual_stroop.js
﻿/************************* 
 * Bilingual_Stroop *
 *************************/
...
let expName = 'bilingual_stroop';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    ...
    {'name': 'english.xlsx', 'path': 'english.xlsx'},
    {'name': 'groupB.xlsx', 'path': 'groupB.xlsx'},
    {'name': 'maori.xlsx', 'path': 'maori.xlsx'},
    {'name': 'groupA.xlsx', 'path': 'groupA.xlsx'},
    {'name': 'english.xlsx', 'path': 'english.xlsx'},
    {'name': 'groupB.xlsx', 'path': 'groupB.xlsx'},
    {'name': 'maori.xlsx', 'path': 'maori.xlsx'},
...
async function experimentInit() {
  ...
  Q = new visual.TextStim({
    win: psychoJS.window,
    name: 'Q',
    text: 'Use the arrow keys to identify the text color. Ignore what the word says.\n\nA reminder will be presented at the top of the screen.\n\nPress space to start.\n',
    font: 'Arial',
    units: undefined, 
  ...
  block_resp = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  
  instrText_4 = new visual.TextStim({
    win: psychoJS.window,
    name: 'instrText_4',
    text: '',
    font: 'Arial',
  ...
  instrText_3 = new visual.TextStim({
    win: psychoJS.window,
    name: 'instrText_3',
    text: '',
    font: 'Arial',
...
function introRoutineBegin(snapshot) {
  return async function () {
    ...
    introComponents = [];
    introComponents.push(background);
...
function instructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    block_resp.keys = undefined;
    block_resp.rt = undefined;
    _block_resp_allKeys = [];
    instrText_4.setText((Instructions + "\nIdentify the COLOR of the text. Ignore what the word says"));
    // keep track of which components have finished
    instructionsComponents = [];
    instructionsComponents.push(background_2);
    instructionsComponents.push(Q);
    instructionsComponents.push(block_resp);
    instructionsComponents.push(instrText_4);
...
function instructionsRoutineEachFrame() {
  return async function () {
    ...
    // *block_resp* updates
    if (t >= 0.0 && block_resp.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      block_resp.tStart = t;  // (not accounting for frame time here)
      block_resp.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { block_resp.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { block_resp.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { block_resp.clearEvents(); });
    }
    
    if (block_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = block_resp.getKeys({keyList: ['space'], waitRelease: false});
      _block_resp_allKeys = _block_resp_allKeys.concat(theseKeys);
      if (_block_resp_allKeys.length > 0) {
        block_resp.keys = _block_resp_allKeys[_block_resp_allKeys.length - 1].name;  // just the last key pressed
        block_resp.rt = _block_resp_allKeys[_block_resp_allKeys.length - 1].rt;
        block_resp.duration = _block_resp_allKeys[_block_resp_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
    ...
    // *instrText_4* updates
    if (t >= 0.0 && instrText_4.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      instrText_4.tStart = t;  // (not accounting for frame time here)
...
function instructionsRoutineEnd(snapshot) {
  return async function () {
    ...
    // update the trial handler
    if (currentLoop instanceof MultiStairHandler) {
      currentLoop.addResponse(block_resp.corr, level);
    }
    psychoJS.experiment.addData('block_resp.keys', block_resp.keys);
    if (typeof block_resp.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('block_resp.rt', block_resp.rt);
        psychoJS.experiment.addData('block_resp.duration', block_resp.duration);
        routineTimer.reset();
        }
...
function trialRoutineBegin(snapshot) {
  return async function () {
    ...
    stim.setColor(new util.Color(wordColor));
    stim.setText(word);
    resp.keys = undefined;
    resp.rt = undefined;
    _resp_allKeys = [];
    trial_counter.setText(((((((("Block " + (blocks.thisN + 1).toString()) + "/") + blocks.nTotal.toString()) + ".   Trial ") + (trials.thisN + 1).toString()) + "/") + trials.nTotal.toString()));
    instrText_2.setText((Instructions + "\nIdentify the COLOR of the text. Ignore what the word says"));
    // keep track of which components have finished
    trialComponents = [];
    trialComponents.push(background_3);
    trialComponents.push(stim);
    trialComponents.push(resp);
    trialComponents.push(trial_counter);
    trialComponents.push(instrText_2);
...
function feedbackRoutineBegin(snapshot) {
  return async function () {
    ...
    fbtxt.setText(fb);
    trial_counter_2.setText(((((((("Block " + (blocks.thisN + 1).toString()) + "/") + blocks.nTotal.toString()) + ".   Trial ") + (trials.thisN + 1).toString()) + "/") + trials.nTotal.toString()));
    instrText_3.setText((Instructions + "\nIdentify the COLOR of the text. Ignore what the word says"));
    // keep track of which components have finished
    feedbackComponents = [];
    feedbackComponents.push(background_4);
    feedbackComponents.push(fbtxt);
    feedbackComponents.push(trial_counter_2);
    feedbackComponents.push(instrText_3);
...
function feedbackRoutineEachFrame() {
  return async function () {
    ...
    // *instrText_3* updates
    if (t >= 0.0 && instrText_3.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      instrText_3.tStart = t;  // (not accounting for frame time here)
    ...
    frameRemains = 0.0 + 1 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (instrText_3.status === PsychoJS.Status.STARTED && t >= frameRemains) {
...
var english_accuracy_percent;
var maori_accuracy_percent;
var endfb;
...
function ThanksRoutineBegin(snapshot) {
  return async function () {
    ...
    // Run 'Begin Routine' code from calc_acc
    english_accuracy_percent = Number.parseInt(((util.sum(english_accuracy) / english_accuracy.length) * 100));
    ...
    thanksText.setText(endfb);
    // keep track of which components have finished
    ThanksComponents = [];
    ThanksComponents.push(background_5);
