...
The "Phone a Friend" task is a general knowledge paradigm in which participants answer trivia questions and can request hints. Each participant has 10 opportunities to "phone a friend" for a hint; of these, 5 hints are correct (valid) and 5 are incorrect (invalid), allowing researchers to study how people use and respond to advice of varying reliability.

The experiment begins with instructions. Participants are presented with questions from a conditions spreadsheet (`conditions.xlsx`), one at a time. For each question, they can either type an answer and press Enter, or press a "phone a friend" key to request a hint. If they request a hint, the pre-assigned hint (either valid or invalid) is displayed.

After receiving a hint, participants can then type their final answer. The task tracks which questions received hints, the hint type (valid/invalid), and the participant's answers both before and after hints. A maximum of 10 hints is enforced across the session, with exactly 5 valid and 5 invalid hints distributed randomly.

The condition file has columns: `this_question` (the question text), `this_cue_valid` (a correct hint), and `this_cue_invalid` (an incorrect hint). The code randomly selects which questions get valid vs. invalid hints while maintaining the 5-5 balance. Data collected includes both pre-hint and post-hint answers, cue type, and calls remaining.

## Key Design Patterns

- **Optional hint mechanism**: participants can choose to request a hint or answer directly, creating two response paths per trial
- **Hint validity manipulation**: 5 valid (correct) and 5 invalid (incorrect) hints randomly assigned across trials, with `calls_remaining` counter tracking usage
- **Text input via `visual.TextBox`**: participants type answers with Enter key to submit, before and/or after receiving hints
- **Conditional hint presentation**: hint text displayed only when participant requests it, using `continueRoutine` logic and state variables (`call_key_pressed`, `hint_requested`)
- **Dual response logging**: `answer.text` (initial response) and `answer_2.text` (response after hint, if any) saved to data
- **Randomized hint selection**: code shuffles which trials get valid vs. invalid hints while maintaining the 5-5 split
