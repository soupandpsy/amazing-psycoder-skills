...
The Wisconsin Card Sorting Task (WCST), based on Grant and Berg (1948), measures executive function, particularly cognitive flexibility and set-shifting. Participants must sort cards according to an unknown rule (color, shape, or number), receiving feedback after each sort. The rule changes after a certain number of correct trials, requiring participants to discover and adapt to the new rule.

The experiment begins with instructions and an example screen showing the three sorting dimensions. Four reference cards are displayed at the top of the screen (e.g., one red triangle, two green stars, three yellow crosses, four blue circles). On each trial, a new card appears at the bottom, and the participant must click one of the four reference cards to sort it.

The sorting rule (color, shape, or number) is initially unknown to the participant. After each sort, feedback ("Correct" or "Incorrect") is displayed. The rule remains consistent until the participant achieves a criterion number of consecutive correct sorts, at which point the rule silently changes. The participant must notice that their previously correct strategy no longer works and discover the new rule.

The card images are pre-rendered JPG files showing various combinations of shape, color, and number. The condition file (`cards.xlsx`) defines all available cards. The code tracks the current rule, consecutive correct responses, and rule changes. The primary measures are the number of categories completed, perseverative errors (continuing with the old rule after a change), and total errors.

## Key Design Patterns

- **Rule-switching paradigm**: hidden sorting rule (color, shape, number) that changes after criterion is met, testing cognitive flexibility
- **Custom rule-tracking code**: JavaScript code tracks `current_rule`, `correct_counter`, `rule_counter`, and handles rule changes programmatically
- **Mouse click responses**: participants click on reference cards to sort, with `eventManager.getMousePos()` and click detection on card positions
- **Four reference card images**: `visual.ImageStim` elements positioned at the top for color, shape, number, and set-size matching categories
- **Feedback after each sort**: text feedback ("Correct"/"Incorrect") displayed briefly, driving rule discovery
- **Card library**: 30+ card images covering all combinations of shapes (triangle, star, cross, circle), colors (red, green, yellow, blue), and numbers (1-4)
