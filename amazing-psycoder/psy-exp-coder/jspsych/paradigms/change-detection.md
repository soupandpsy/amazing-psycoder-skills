...
The Change Detection task measures visual working memory capacity by testing the ability to detect changes between two briefly presented visual displays. Participants view a sample array of colored circles, followed by a brief retention interval, then a test array that is either identical to the sample or has one changed item.

The experiment has two phases. In the first phase (change detection), participants see a memory array of colored circles for a brief period, followed by a blank retention interval, and then a test array. They indicate whether any circle changed color (same/different judgment). The set size (number of circles) varies across trials.

The second phase (localisation) presents a similar paradigm but requires participants to identify which specific circle changed, rather than simply detecting a change. This localisation measure provides a more sensitive assay of visual working memory precision.

Stimulus positions and colors are defined in CSV files (`sampled_circle_points_with_colors.csv` and `localisation_trials_with_test_columns.xlsx`). Circles are rendered as `visual.ShapeStim` elements at predefined angular positions around a central fixation point. The code programmatically generates circles with specified colors using RGB values from the condition files.

The experiment includes instructions before each phase, a progress counter, and a conclusion screen. Accuracy and response time are recorded per trial, with change detection performance typically analyzed as a function of set size.

## Key Design Patterns

- **Two-phase experimental design**: change detection phase followed by localisation phase, each with its own trial loop, instructions, and condition files
- **Programmatic stimulus generation**: colored circles are created dynamically using `visual.ShapeStim` at calculated angular positions around fixation, rather than pre-rendered images
- **CSV-driven trial parameters**: `sampled_circle_points_with_colors.csv` defines circle positions and colors; `localisation_trials_with_test_columns.xlsx` defines test parameters
- **Multi-component trial structure**: fixation cross, memory array, retention interval (blank), test array, and response collection, each timed precisely using the routine's frame counter
- **`ShapeStim` with dynamic vertices** for rendering circles at specific positions and colors, updated per trial via code components
- **Set size manipulation**: number of circles varies across trials to measure working memory capacity limits (typically 2-6 items)
