...
The Mental Rotation task, based on Shepard and Metzler's classic paradigm, measures the time participants take to determine whether two presented shapes are identical or mirror images, as a function of the angular rotation between them. Reaction time typically increases linearly with rotation angle, peaking at 180 degrees.

The experiment begins with two instruction screens explaining the task. Participants see two letter-like shapes (e.g., 'F') presented side by side. One shape may be rotated relative to the other at various angles. The participant's task is to decide whether the two shapes are the same (identical) or different (mirror images).

Each trial presents the pair of shapes and waits for a key response ('s' for same, 'd' for different). The condition file (`MentalRot.csv`) defines the rotation angle, which image to display for each position, and the correct answer. Images include the original letter (`F.png`) and its mirror-reversed version (`FR.png`).

Reaction time is measured from stimulus onset. Data is saved per trial including RT, accuracy, rotation angle, and response. After all trials, a conclusion screen is shown. The classic finding is a linear relationship between rotation angle and RT, reflecting the mental process of rotating one object into alignment with another.

## Key Design Patterns

- **Two-instruction-screen design**: `Intructions_1` and `Intructions_2` routines providing incremental task explanation
- **CSV condition file** (`MentalRot.csv`) specifying rotation angle, image files for left/right positions, and correct answer
- **Same/different judgment**: binary choice using 's' (same) and 'd' (different) keys with `corrAns` comparison for accuracy
- **Image pair presentation**: two `visual.ImageStim` elements (left and right) with rotation applied via the condition file's angle parameter
- **Rotation angle manipulation**: condition file varies angular disparity systematically (e.g., 0, 45, 90, 135, 180 degrees)
- **`TrialHandler` with SEQUENTIAL or RANDOM method** iterating over `MentalRot.csv` entries
...
// Original file: MentalRotation.js
﻿/*********************** 
 * Mentalrotation *
 ***********************/
