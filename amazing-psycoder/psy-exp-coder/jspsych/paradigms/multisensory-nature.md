# Multisensory Nature — PsychoJS (Online)
...
> **Source**: [Pavlovia demos/multisensory_nature](https://gitlab.pavlovia.org/demos/multisensory_nature) · PsychoJS (PsychoPy online export)
> **Platform note**: This is PsychoJS code (PsychoPy's JavaScript runtime for online experiments), NOT jsPsych library code.

## Experiment Logic

The Multisensory Nature task, based on Aldoh et al. (2023), investigates how visual and auditory nature exposure interact to influence positive and negative affect. Participants view audiovisual recordings that vary in the proportion of natural vs. urban visual scenes and soundscapes, while rating their emotional state.

The experiment begins with instructions and a pre-task I-PANAS-SF affect questionnaire. The main trial loop presents 15 audiovisual clips, each 60 seconds long, varying in visual nature level and sound type (natural sounds, anthropogenic noise, or mixed). Participants indicate their positive and negative affect while viewing each clip.

Each trial loads a video file (e.g., N1.mp4, T3.mp4, R5.mp4) specified by the condition file (vids.xlsx). The video plays for the full 60-second duration using PsychoJS's visual.MovieStim. After each clip, participants complete affect ratings using slider components.

The condition file includes columns for the video filename, visual nature level, and sound type. Data collected includes affect ratings, video identity, and condition parameters. The key findings from the original study show that high visual nature with low anthropogenic noise produces the highest positive affect and lowest negative affect, with individual differences in nature connectedness moderating these effects.

## Key Design Patterns

- **Audiovisual stimulus presentation**: visual.MovieStim with sound.Sound for combined video-audio playback
- **60-second trial duration**: each clip plays for a full minute with continuous affect monitoring
- **Slider-based affect ratings**: visual.Slider components for positive and negative affect ratings after each video
- **I-PANAS-SF questionnaire**: pre-task affect measure loaded from Excel file to control for baseline mood
- **Multiple video resources**: 15+ MP4 files preloaded as experiment resources covering nature (N), urban (T), and mixed (R) scenes
- **Condition-driven video selection**: vids.xlsx specifies which video file to play per trial with associated condition labels
...
// Original file: multisensory_nature.js
﻿/**************************** 
 * Multisensory_Nature *
 ****************************/
...
let expName = 'multisensory_nature';  // from the Builder filename that created this script
let expInfo = {
