...
The psychophysics staircase demo measures the minimal orientation difference at which a participant can distinguish between two gratings. It implements an adaptive staircase procedure entirely in custom JavaScript code, since PsychoJS did not have a built-in staircase method at the time this demo was created.

The experiment begins with instructions. A custom staircase setup in the "Before Experiment" code configures all parameters: starting value (70 degrees), step sizes ([10, 5, 2, 1, 0.5]), number of reversals (5), up/down rules (1-up 1-down), and bounds (min 0, max 90). These parameters control the adaptive tracking of the orientation difference threshold.

On each trial, two grating images are presented: one on the left and one on the right. One grating is tilted slightly clockwise, the other counterclockwise. The participant must indicate which side has the clockwise-tilted grating, pressing the left or right arrow key. The grating images are pre-rendered PNGs (`grating_cropped.png`) rotated via the `ori` property.

Based on the participant's response, the staircase updates the orientation difference (`level`): correct responses make the difference smaller (harder), incorrect responses make it larger (easier). A reversal occurs when the direction changes. After a set number of reversals, the threshold is calculated as the average of the reversal values. The experiment ends with a screen showing the estimated threshold.

## Key Design Patterns

- **Custom staircase implementation**: no built-in `MultiStairHandler`; all staircase logic is hand-coded using variables for `level`, `currentDirection`, `stepSizes`, `nReversals`, `nUp`, `nDown`, and `reversalVals`
- **Adaptive difficulty**: orientation difference (`level`) increases/decreases based on response accuracy according to a 1-up 1-down rule
- **Step size progression**: step sizes decrease through `[10, 5, 2, 1, 0.5]` across successive reversals for fine-grained threshold estimation
- **Grating stimuli**: `visual.ImageStim` with the `ori` property dynamically set to present opposing tilt directions
- **Two-alternative forced choice (2AFC)**: left vs. right arrow keys for clockwise-tilt identification
- **Threshold calculation**: average of all reversal values after `nReversals` reversals, displayed on the end screen
