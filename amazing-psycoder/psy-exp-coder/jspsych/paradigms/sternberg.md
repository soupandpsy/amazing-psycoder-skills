...
The Sternberg Working Memory Task, based on Sternberg (1969), measures memory scanning speed by varying the number of items held in working memory (set size). Participants memorize a set of digits, then respond whether a probe digit was present in the memorized set. The critical finding is that reaction time increases linearly with set size, suggesting serial (rather than parallel) memory search.

The experiment has practice and main phases, each with its own instruction screen. On each trial, a memory set of 1-6 digits is presented sequentially or simultaneously. After a brief retention interval, a single probe digit appears. The participant presses one key if the probe was in the memory set (present) and another key if it was not (absent).

The condition files (`pracTrials.xlsx` and `mainTrials.xlsx`) define the memory set, probe digit, correct response, and set size for each trial. Set size varies to test the linear relationship between RT and memory load. The probe can be present or absent, and can appear at any serial position within the set.

Reaction time is measured from probe onset. Accuracy is determined by comparing `resp.keys` to `corrAns`. The experiment concludes with an end screen. The classic analysis plots mean RT as a function of set size, separately for present and absent trials. A linear increase supports serial exhaustive search; a flat function supports parallel search. The slope difference between present and absent trials tests whether the search is self-terminating.

## Key Design Patterns

- **Set size manipulation**: memory set size varies (typically 1-6 items), with `set_size` column in condition files driving the independent variable
- **Two-phase design**: practice block (`pracTrials`) with feedback followed by main experimental block (`trials`) without feedback
- **Sequential or simultaneous memory set presentation**: digits displayed using `visual.TextStim` with timed onset/offset for each item in the set
- **Probe recognition**: binary response (present/absent) via keyboard, with `corrAns` comparison for accuracy
- **Dual condition files**: `pracTrials.xlsx` and `mainTrials.xlsx` define trial parameters including memory set items, probe, and correct answer
- **Serial position tracking**: condition file can specify which position the probe occupies in the set for present trials
