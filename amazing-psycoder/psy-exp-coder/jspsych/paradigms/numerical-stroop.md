...
The Numerical Stroop task, based on Henik and Tzelgov (1982), is a variant of the classic Stroop paradigm using numerical stimuli instead of color words. Participants must compare numbers along two dimensions -- physical size and numerical magnitude -- which may be congruent or incongruent.

The experiment has two block types. In physical trials, participants select the physically larger number while ignoring its numerical value. In semantic trials, participants select the numerically larger number while ignoring its physical size. This creates congruent trials (e.g., physically larger 5 vs. smaller 3) and incongruent trials (e.g., physically larger 3 vs. smaller 5).

The experiment is structured with an outer block loop (`blocks`) that presents physical and semantic blocks in counterbalanced order. Each block begins with specific instructions. Practice trials with feedback precede each block type. The condition files (`practice_physical.xlsx`, `stroop_input_physical.xlsx`, `practice_semantic.xlsx`, `stroop_input_semantic.xlsx`) define trial parameters.

Two numbers are presented side by side as `visual.TextStim` elements with varying font sizes. Participants press '1' for the left number or '2' for the right number. RT and accuracy are recorded per trial. The typical finding is longer RT on incongruent vs. congruent trials (the numerical Stroop effect).

## Key Design Patterns

- **Two-dimension Stroop design**: physical size judgment vs. numerical magnitude judgment, creating a double dissociation
- **Block-level counterbalancing**: `blockDefinitions.xlsx` specifies which block type comes first, with dual practice-then-experiment structure for each
- **Four condition files**: separate Excel files for practice-physical, experiment-physical, practice-semantic, and experiment-semantic trials
- **Dynamic text sizing**: `visual.TextStim` with `letterHeight` set per trial to manipulate physical size independently from numerical value
- **Binary choice response**: keys '1' (left) and '2' (right) for selecting which number is larger
- **Block instructions**: specific instruction text for each block type explaining whether to judge physical size or numerical magnitude

## Code Example

```javascript
// Source: numerical_stroop (demos/numerical_stroop)
// Project URL: https://gitlab.pavlovia.org/demos/numerical_stroop
// Original file: NumericalStroop.js
﻿/************************ 
