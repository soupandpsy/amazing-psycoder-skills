...
> **Config reference**: [drag-and-drop](../../../psy-exp-designer/paradigms/drag-and-drop.md)
...
The Drag and Drop Puzzle task demonstrates PsychoPy's drag-and-drop interaction capabilities through a pattern-matching puzzle game. Participants are presented with a target pattern displayed above an empty grid and must drag black and white puzzle pieces into the grid to recreate the pattern.

The experiment begins with an instructions screen. In each trial, the target design (e.g., `design_92.png`) is displayed above a grid of empty cells. Draggable puzzle piece images (black and white squares) are positioned around the grid. Participants click and drag each piece to its correct grid position.

When the participant is satisfied with their arrangement, they press a "Continue" button. Feedback is then displayed showing whether the arrangement was correct and the time taken to complete the trial. The condition file (`conditions.xlsx`) defines the target design and the correct arrangement for each trial.

The task uses PsychoJS `visual.ImageStim` with `draggable: true` property. Piece positions are tracked using each stim's `.pos` property. Correctness is determined by comparing dragged piece positions to the target pattern defined in the condition file. Data collected includes trial completion time, accuracy, and final piece positions.

## Key Design Patterns

- **Drag-and-drop interaction**: `visual.ImageStim` with `setDraggable(true)` enabling mouse-based drag operations
- **Puzzle grid design**: target pattern image displayed above an empty grid, with black/white piece images as draggable elements
- **Custom piece placement verification**: code compares final dragged positions against target pattern from condition file columns (`a1` through `a9`)
- **"Continue" button**: `visual.ImageStim` (`continueButton.png`) that ends the trial when clicked, allowing participant self-pacing
- **Pixels units**: window uses `units: 'pix'` for precise drag-and-drop positioning
- **Trial timer**: elapsed time displayed at trial end, measuring how long the participant took to complete the puzzle
