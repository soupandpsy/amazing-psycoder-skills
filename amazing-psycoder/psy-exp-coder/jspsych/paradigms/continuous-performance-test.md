...
> **Config reference**: [continuous-performance-test](../../../psy-exp-designer/paradigms/continuous-performance-test.md)
...
The Continuous Performance Test (CPT) is a go/no-go task that measures sustained attention and response inhibition. Participants view a rapid stream of letter stimuli and must respond to every letter except the target letter 'X'. This requires maintaining vigilance over an extended period while inhibiting the prepotent response to the infrequent no-go target.

The experiment begins with an instructions screen explaining the task. The main trial loop presents a stream of letters, one at a time. Each trial begins with a fixation cross displayed for 1 second, followed by a letter stimulus for 1 second. The participant must press the spacebar for every letter except 'X'.

The condition file (`conditions.xlsx`) specifies the letter to display (`letter` column) and the correct answer (`corrAns` column: 'space' for go trials, 'none' for no-go/X trials). A trial counter shows progress throughout the experiment. Accuracy is tracked using a `correct_counter` that increments for correct responses.

The dual-path accuracy logic handles both response types: when a key is pressed, accuracy is checked against `corrAns`; when no key is pressed, the code checks whether `corrAns` is 'none'. The experiment ends with a screen showing the total score. CPT performance is typically measured by omission errors (missed go responses) and commission errors (false alarms to no-go stimuli).

## Key Design Patterns

- **Go/No-go paradigm with letter stimuli**: spacebar press required for all letters except 'X', creating a response inhibition challenge
- **Two-part trial structure**: fixation cross (1s) followed by letter stimulus (1s), with keyboard listening during the letter phase
- **Dual-path accuracy logic**: explicit `response.keys == corrAns` comparison when a key is pressed; `corrAns === 'none'` check when no response made
- **Trial counter**: `visual.TextStim` showing current trial / total trials for participant pacing
- **`correct_counter` accumulator**: initialized in Begin Experiment, incremented per correct trial, displayed as final score
- **Condition file with `letter` and `corrAns` columns** controlling stimulus identity and expected response per trial

## Code Example

```javascript
// Source: continuous_performance_test (demos/continuous_performance_test)
// Project URL: https://gitlab.pavlovia.org/demos/continuous_performance_test
// Original file: continuous_performance_test.js
﻿/************************************ 
...
let expName = 'continuous_performance_test';  // from the Builder filename that created this script
let expInfo = {
    'participant': '',
    'session': '001',
};
...
async function experimentInit() {
  ...
  instructions = new visual.TextStim({
    win: psychoJS.window,
    name: 'instructions',
    text: 'In this task you will be required to attend to the presented letter stimuli. Press space after every letter stimulus except from letter X. If you see X please do not respond. Reaction time is measured so please respond as quickly as possible. Press space to continue.',
    font: 'Arial',
    units: undefined, 
