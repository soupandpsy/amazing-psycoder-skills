...
The Choice Reaction Time task measures the speed of decision-making when participants must select among multiple response options based on a presented stimulus. Unlike simple reaction time (one stimulus, one response), choice RT requires stimulus discrimination and response selection.

The experiment begins with an instructions screen. A practice block with feedback is presented first, followed by a second instruction screen, then the main experimental block. Each trial shows a target stimulus (one of several shapes: cross, plus, square, or I/T shapes) at the center of the screen. Participants must press the corresponding key that matches the shape.

The response mapping uses labeled response images displayed on screen, showing which key corresponds to which shape. Stimuli are loaded as image files from an `images/` directory. The condition file (`RTtimeConditions.xlsx`) defines which target image to display and the correct response mapping.

Reaction time is measured using a keyboard component with clock reset at stimulus onset. After responding, a ready signal indicates the next trial can begin. The experiment includes a fixation period before each stimulus. Data collected includes reaction time, accuracy, and the specific key pressed.

## Key Design Patterns

- **Multi-alternative choice design**: 3-4 possible stimulus shapes (cross, plus, square, I, T) each mapped to a specific key response
- **Response mapping display**: `visual.ImageStim` showing labeled response keys on screen as a reference for participants
- **Two-block structure**: practice with feedback (`PracticeLoop`) followed by main experiment (`ExptLoop`) with separate condition file loads
- **Fixation-to-stimulus sequence**: grey square fixation transitioning to target stimulus, with `ready_signal` image between trials
- **Ready signal inter-trial interval**: `response_ready.jpg` displayed between trials to pace the experiment
- **Image-based stimuli**: all targets and response cues are pre-rendered JPG images rather than drawn text
