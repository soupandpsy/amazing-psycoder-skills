...
> **Config reference**: [corsi-blocks](../../../psy-exp-designer/paradigms/corsi-blocks.md)
...
The Corsi Blocks task is a visuospatial working memory test that measures the capacity to remember and reproduce sequences of spatial locations. Nine blocks are arranged in an irregular pattern on screen. The computer highlights blocks in a sequence, and the participant must click the blocks in the same order.

The experiment begins with an instructions screen. The main task loop is self-contained within a single routine (`corsi_task`) that programmatically generates sequences of increasing length. Blocks are rendered as `visual.Rect` or `visual.ShapeStim` elements at fixed positions. The sequence is presented by briefly changing each block's color in order.

During the presentation phase, blocks flash in a predetermined sequence with a fixed stimulus onset asynchrony. During the recall phase, participants click blocks using the mouse to reproduce the sequence. The mouse position is monitored using `psychoJS.eventManager.getMousePos()` to detect clicks on block locations.

Sequence length starts at 2 and increases with successful reproduction (span increase) or decreases with failure (span decrease). The task continues until a stopping criterion is met. The primary measure is the Corsi span -- the longest sequence length the participant can reliably reproduce -- and total correct trials.

## Key Design Patterns

- **Custom adaptive sequencing**: sequence generation, presentation, and recall logic all implemented in code components rather than condition files
- **Mouse interaction**: `eventManager.getMousePos()` and click detection for participants to select blocks during recall
- **Programmatic block rendering**: `visual.Rect` elements positioned in the classic Corsi 9-block spatial arrangement
- **Self-contained routine**: the entire Corsi task (presentation + recall phases) runs within a single `corsi_task` routine using state tracking variables
- **Adaptive difficulty**: sequence length increases/decreases based on performance using custom variables (`current_span`, `correct_counter`, `trials_at_span`)
- **Two-phase within-trial structure**: presentation phase (auto-timed block flashing) followed by recall phase (mouse-driven response collection)

## Code Example

```javascript
// Source: corsi_blocks (demos/corsi_blocks)
// Project URL: https://gitlab.pavlovia.org/demos/corsi_blocks
// Original file: corsi_blocks.js
﻿/********************* 
 * Corsi_Blocks *
 *********************/
