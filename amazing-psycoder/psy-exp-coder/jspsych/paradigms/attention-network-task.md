...
> **Config reference**: [attention-network-task](../../../psy-exp-designer/paradigms/attention-network-task.md)
...
The Attention Network Task (ANT), based on Fan et al. (2002), measures three attention networks -- alerting, orienting, and executive control -- within a single 30-minute testing session. The experiment presents participants with a series of trials where they must respond to the direction of a central arrow target flanked by congruent, incongruent, or neutral flanker stimuli.

Each trial begins with one of several cue conditions: no cue, center cue, double cue, or spatial cue (upper/lower). The cue provides information about when and where the target will appear. After a variable cue-target interval, a row of five arrows is presented. The participant's task is to indicate whether the central arrow points left or right, ignoring the flanking arrows.

The experimental flow includes two instruction screens, followed by a trial loop that presents all combinations of cue type and flanker type. Stimuli are pre-rendered as PNG images (`congLeft.png`, `incongRight.png`, etc.) loaded from a `stim/` directory. The condition file (`cond.xlsx`) specifies which stimulus image to display per trial, along with the correct key response.

Reaction time is measured using `keyboard.Keyboard` with its clock reset via `callOnFlip` at stimulus onset. Accuracy is determined by comparing `key_resp.keys` to the `corrAns` column. At the end, participants see a completion screen.

## Key Design Patterns

- **Pre-rendered stimulus images** rather than text-drawn stimuli, with 12+ PNG files covering all cue-target-flanker combinations loaded as resources
- **Two separate instruction routines** (`instr` and `instr2`) providing incremental task explanation before the main trials
- **`Scheduler`-based flow**: `flowScheduler.add(Begin())`, `flowScheduler.add(EachFrame())`, `flowScheduler.add(End())` pattern for every routine
- **Frame-accurate component timing**: stimuli are drawn using `setAutoDraw(true)` with duration controlled by `t >= startTime + duration - frameTolerance`
- **Comprehensive resource preloading**: all stimulus images, background, and default fallback images declared in `psychoJS.start()` resources array
- **`TrialHandler` with RANDOM method** iterating over `cond.xlsx`, with per-trial data saved via `psychoJS.experiment.addData()`

## Code Example

```javascript
// Source: attention_network_task (demos/attention_network_task)
// Project URL: https://gitlab.pavlovia.org/demos/attention_network_task
// Original file: attention_network_task.js
﻿/******************************* 
