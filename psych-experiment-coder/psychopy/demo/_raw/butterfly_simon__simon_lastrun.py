...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/Becca/Library/CloudStorage/GoogleDrive-becca@opensciencetools.org/Shared drives/Science/Pavlovia Demos/simon_task/simon_lastrun.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win.color = [0.3569, 0.6941, 0.8039]
        win.colorSpace = 'rgb'
        win.backgroundImage = ''
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    background = visual.ImageStim(
        ...
        image='images/background-1877877_1280.jpg', mask=None, anchor='center',
    ...
    left_image_2 = visual.ImageStim(
        ...
        ori=0.0, pos=(-0.2, 0), draggable=False, size=(0.3, 0.3),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
    ...
    instructions = data.Routine(
        name='instructions',
        components=[background, instr_textbox, start_button],
    )
    ...
    for thisTrial_2 in trials_2:
        ...
        for thisLoop_until_correct in loop_until_correct:
            ...
            practice_trial = data.Routine(
                name='practice_trial',
                components=[background_2, practxt, left_image_2, right_image_2, mouse_2],
            )
            ...
            while continueRoutine:
                ...
                # if practxt is starting this frame...
                if practxt.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    practxt.frameNStart = frameN  # exact frame index
                    practxt.tStart = t  # local t and not account for scr refresh
                    practxt.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(practxt, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'practxt.started')
                    # update status
                    practxt.status = STARTED
                    practxt.setAutoDraw(True)
                
                # if practxt is active this frame...
                if practxt.status == STARTED:
                    # update params
                    pass
                ...
                if left_image_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if right_image_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if mouse_2.status == STARTED:  # only update if started and not finished!
                    ...
                    if buttons != prevButtonState:  # button state changed?
                        ...
                        if sum(buttons) > 0:  # state changed to a new click
                            ...
                            gotValidClick = False
                            clickableList = environmenttools.getFromNames([left_image_2, right_image_2], namespace=locals())
            ...
            feedback = data.Routine(
                name='feedback',
                components=[background_6, background_3, fb_txt],
            )
            ...
            while continueRoutine and routineTimer.getTime() < 0.5:
                ...
                # if background_6 is stopping this frame...
                if background_6.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > background_6.tStartRefresh + 0.5-frameTolerance:
                        # keep track of stop time/frame for later
                        background_6.tStop = t  # not accounting for scr refresh
                        background_6.tStopRefresh = tThisFlipGlobal  # on global time
                        background_6.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'background_6.stopped')
                        # update status
                        background_6.status = FINISHED
                ...
                if background_3.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > background_3.tStartRefresh + 0.5-frameTolerance:
                        # keep track of stop time/frame for later
    ...
    instructions_2 = data.Routine(
        name='instructions_2',
        components=[background_4, instr_textbox_2, start_button_2],
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[background_5, left_image, right_image, mouse],
        )
            ...
            if mouse.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        # check if the mouse was inside our 'clickable' objects
                        gotValidClick = False
                        clickableList = environmenttools.getFromNames([left_image, right_image], namespace=locals())
                        for obj in clickableList:
                            # is this object clicked on?
